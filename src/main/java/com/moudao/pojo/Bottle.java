...
public class Bottle {
    ...
    @NotBlank(groups = {InsertValid.class}, message = "瓶子的标题不能为空")
    @Size(min = 1, max = 60, message = "瓶子的标题长度不得超过60个字符")
    private String bottleTitle;

    @NotBlank(groups = {InsertValid.class}, message = "瓶子的内容不得为空")
    @Size(min = 1, max = 200, message = "瓶子的内容不得超过200个字符")
    private String bottleContent;

    @NotNull(groups = {InsertValid.class}, message = "瓶子的类型不能为空，0：作业求解瓶，1：知识问答瓶")
    @Min(value = 0, message = "瓶子的类型只能是，0：作业求解瓶，1：知识问答瓶")
    @Max(value = 1, message = "瓶子的类型只能是，0：作业求解瓶，1：知识问答瓶")
    private Byte bottleCategory;

    @Min(value = 0, message = "是否是优选瓶子：0：普通瓶子，1：优选瓶子")
    @Max(value = 1, message = "是否是优选瓶子：0：普通瓶子，1：优选瓶子")
    private Byte bottleStatus;

    @NotNull(groups = {InsertValid.class}, message = "创建这个瓶子的人的id不能为空")
    @Min(value = 1, message = "创建人的id的位数在1到11之间")
    @Digits(integer = 11, fraction = 0, message = "创建人的id的位数在1到11之间")
